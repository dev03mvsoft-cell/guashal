    ...
    <main class="flex-1 p-6 lg:p-12 overflow-y-auto">
        <div class="max-w-7xl mx-auto">
            <header class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
                <div>
                    <span class="text-saffron font-black uppercase tracking-[0.3em] text-[13px] mb-2 block">Council Registry</span>
                    <h1 style="font-family: 'Playfair Display';" class="text-4xl font-bold text-nature leading-tight">The <span class="text-saffron italic">Visionaries</span></h1>
                    <p class="text-nature/40 mt-1 text-[13px] font-medium tracking-wide">Registry of founding souls and executive trustees</p>
                </div>
                <div class="flex items-center gap-4">
                    <form id="bulk-form" method="POST" onsubmit="return confirmAction(event, 'Purge selected visionaries?', 'The selected leaders will be removed from the digital council.');">
                    ...
                    <a href="editor.php" class="bg-nature text-white px-8 py-4 rounded-xl font-bold flex items-center gap-3 shadow-lg shadow-nature/20 hover:scale-105 transition-all">
                        <i class="fas fa-plus text-xs"></i> <span>Honor Visionary</span>
                    </a>
                </div>
            </header>
            ...
            <?php if (isset($_GET['msg'])): ?>
                <div class="bg-nature/10 text-nature p-4 rounded-xl mb-8 font-bold text-sm border border-nature/20 animate-fade-in shadow-sm shadow-nature/5">
                    <i class="fas fa-certificate mr-2"></i> <?= htmlspecialchars($_GET['msg']) ?>
                </div>
            <?php endif; ?>
            ...
            <div class="mb-6 flex items-center px-4">
                <label class="flex items-center gap-3 cursor-pointer group">
                    ...
                    <span class="text-[12px] font-black uppercase tracking-widest text-nature/40 group-hover:text-nature transition-colors">Select All Leaders</span>
                </label>
            </div>
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-24">
                                    <div class="flex flex-col items-center">
                                        <div class="w-20 h-20 bg-nature/5 text-nature/10 rounded-full flex items-center justify-center text-4xl mb-4 italic">V</div>
                                        <p class="text-nature/30 uppercase font-black tracking-widest text-[11px]">The founding circle is silent</p>
                                    </div>
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($items as $item): ?>
                                <tr class="group">
                                    ...
                                    <td>
                                        ...
                                        <div class="text-[12px] text-nature/50 font-normal uppercase tracking-tighter italic">
                                            <?= htmlspecialchars($item['designation_en']) ?> | <?= (strcasecmp($item['type'], 'founder') == 0) ? 'Founder Member' : 'Trustee Board' ?>
                                        </div>
                                    </td>
                                    <td>
                                        <span class="px-3 py-1.5 rounded-full text-[11px] font-medium uppercase tracking-widest <?= (strcasecmp($item['type'], 'founder') == 0) ? 'bg-gold/10 text-nature border border-gold/20' : 'bg-nature/10 text-nature border border-nature/5' ?>">
                                            <?= (strcasecmp($item['type'], 'founder') == 0) ? 'Founder' : 'Trustee' ?>
                                        </span>
                                    </td>
                                    <td>
                                        <div class="w-10 h-10 bg-slate-50 border border-slate-100 rounded-lg flex items-center justify-center font-normal text-slate-400 text-xs text-center">
                                            <?= $item['sort_order'] ?>
                                        </div>
                                    </td>
                                    <td class="pr-6 text-right">
                                        <div class="flex justify-end items-center gap-3">
                                            ...
                                            <form method="POST" onsubmit="return confirmAction(event, 'Release Visionary?', 'This member will be removed from the council.');" class="inline">
