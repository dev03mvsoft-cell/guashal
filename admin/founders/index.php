...
$items = [];
try {
    $items = $pdo->query("SELECT * FROM founders ORDER BY sort_order ASC, created_at DESC")->fetchAll();
} catch (Exception $e) {
}
...
<head>
    ...
    <title>The Visionaries - Gaushala Admin</title>
    ...
    <main class="flex-1 p-6 md:p-12 overflow-y-auto h-full">
        <div class="max-w-7xl mx-auto">
            ...
            <header class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-20 gap-8">
                <div>
                    <h1 style="font-family: 'Playfair Display';" class="text-6xl font-bold text-nature mb-4 tracking-tight">The <span class="italic text-[#FFD700]">Visionaries</span></h1>
                    <div class="flex items-center gap-6 text-[12px] tracking-[0.4em] font-black text-nature/40 uppercase">
                        <span>Founding Souls</span>
                        <div class="w-1.5 h-1.5 bg-[#FF6A00] rounded-full animate-pulse"></div>
                        <span class="text-[#FF6A00]"><?= count($items) ?> Divine Leaders</span>
                    </div>
                </div>
                <div class="flex items-center gap-6">
                    <form id="bulk-form" method="POST" onsubmit="return confirmAction(event, 'Purge selected?', 'The selected visionaries will be removed from the council.');">
                    ...
                    <a href="editor.php" class="bg-nature text-white px-10 py-5 rounded-[2rem] font-bold text-[15px] shadow-2xl hover:shadow-[#FFD700]/20 hover:bg-[#FFD700] hover:text-nature transition-all flex items-center gap-4 group">
                        <i class="fas fa-plus-circle text-xl group-hover:rotate-90 transition-transform"></i>
                        <span>Onboard Visionary</span>
                    </a>
                </div>
            </header>
            ...
            <div class="mb-12 flex items-center px-4">
                <label class="flex items-center gap-3 cursor-pointer group">
                    ...
                    <span class="text-[12px] font-black uppercase tracking-[0.3em] text-nature/20 group-hover:text-nature transition-colors">Select All Leaders</span>
                </label>
            </div>

            <?php if (isset($_GET['msg'])): ?>
                <div class="bg-white border-l-8 border-gold p-8 rounded-[2.5rem] mb-16 shadow-2xl flex items-center gap-6 animate-slide-in">
                    <div class="w-12 h-12 bg-gold/5 text-nature rounded-full flex items-center justify-center text-xl shadow-inner italic font-display">V</div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-12">
                <?php if (empty($items)): ?>
                    <div class="col-span-full py-40 text-center glass rounded-[4rem] border-2 border-dashed border-nature/10">
                        <i class="fas fa-user-tie text-9xl text-nature/5 mb-10"></i>
                        <h2 class="text-2xl font-display italic text-nature/30 uppercase tracking-[0.2em]">The founding circle is silent...</h2>
                        <a href="editor.php" class="text-[#FF6A00] font-black uppercase text-[12px] tracking-widest mt-8 inline-block hover:underline">Honor a Visionary <i class="fas fa-arrow-right ml-2 text-[12px]"></i></a>
                    </div>
                <?php else: ?>
                    <?php foreach ($items as $item): ?>
                        <div class="founder-card glass p-8 rounded-[3.5rem] bg-white border border-nature/5 flex flex-col items-center text-center group relative overflow-hidden h-full">
                            ...
                            <!-- Actions Overlay -->
                            <div class="mt-10 flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-10 group-hover:translate-y-0 relative z-10">
                                <a href="editor.php?id=<?= $item['id'] ?>" class="w-12 h-12 bg-nature text-white rounded-2xl flex items-center justify-center hover:bg-gold hover:text-nature transition-all shadow-xl"><i class="fas fa-pen text-sm"></i></a>
                                <form method="POST" class="inline" onsubmit="return confirmAction(event, 'Release Visionary?', 'This will remove them from the digital council.');">
