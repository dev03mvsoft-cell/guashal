...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'delete') {
        ...
        $stmt->execute([$id]);
        header("Location: index.php?msg=Voice Removed");
        exit;
    }
    ...
    if ($_POST['action'] === 'bulk_delete' && !empty($_POST['selected_ids'])) {
        ...
        $stmt = $pdo->prepare("DELETE FROM testimonials WHERE id IN ($placeholders)");
        $stmt->execute($ids);
        header("Location: index.php?msg=" . count($ids) . " Voices Cleared");
        exit;
    }
}
...
<head>
    ...
    <title>Devotee Stories Registry</title>
    ...
    <main class="flex-1 p-6 lg:p-12 overflow-y-auto">
        <div class="max-w-7xl mx-auto">
            <header class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
                <div>
                    <span class="text-saffron font-black uppercase tracking-[0.3em] text-[13px] mb-2 block">Echoes of Devotion</span>
                    <h1 style="font-family: 'Playfair Display';" class="text-4xl font-bold text-nature leading-tight">Devotee <span class="text-saffron italic">Stories</span></h1>
                    <p class="text-nature/40 mt-1 text-[13px] font-medium tracking-wide">Registry of public expressions of faith and gratitude</p>
                </div>
                <div class="flex items-center gap-4">
                    <form id="bulk-form" method="POST" onsubmit="return confirmAction(event, 'Purge selected stories?', 'The selected devotee voices will be deleted forever.');">
                    ...
                    <a href="editor.php" class="bg-saffron text-white px-8 py-4 rounded-xl font-bold flex items-center gap-3 shadow-lg shadow-saffron/20 hover:scale-105 transition-all">
                        <i class="fas fa-plus text-xs"></i> <span>Publish Story</span>
                    </a>
                </div>
            </header>
            ...
            <?php if (isset($_GET['msg'])): ?>
                <div class="bg-nature/10 text-nature p-4 rounded-xl mb-8 font-bold text-sm border border-nature/20 animate-fade-in shadow-sm shadow-nature/5">
                    <i class="fas fa-quote-left mr-2"></i> Voice registry successfully updated.
                </div>
            <?php endif; ?>
                ...
                <table class="system-table">
                    <thead>
                        <tr>
                            ...
                            <th>Devotee Information</th>
                            <th>Testimony Snippet</th>
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-24">
                                    <div class="flex flex-col items-center">
                                        <div class="w-20 h-20 bg-nature/5 text-nature/10 rounded-full flex items-center justify-center text-4xl mb-4 italic"><i class="fas fa-quote-right"></i></div>
                                        <p class="text-nature/30 uppercase font-black tracking-widest text-[11px]">The sanctuary awaits its first echo</p>
                                    </div>
                                </td>
                            </tr>
                        <?php else: ?>
                            ...
                            ?>
                                <tr class="group">
                                    ...
                                    <td>
                                        <div class="w-10 h-10 bg-nature/5 rounded-full flex items-center justify-center font-black text-nature/20 border border-nature/5">
                                            <?= $initial ?>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="font-medium text-nature text-[16px] leading-tight mb-0.5"><?= htmlspecialchars($item['name']) ?></div>
                                        <div class="text-[11px] font-normal uppercase text-saffron tracking-widest"><?= htmlspecialchars($item['role']) ?></div>
                                    </td>
                                    <td>
                                        <div class="text-[14px] text-nature/60 line-clamp-1 italic italic">"<?= htmlspecialchars($item['testimonial']) ?>"</div>
                                    </td>
                                    <td>
                                        <div class="flex gap-0.5 text-gold text-[10px]">
                                            <?php for ($i = 0; $i < $item['rating']; $i++): ?><i class="fas fa-star"></i><?php endfor; ?>
                                        </div>
                                    </td>
                                    <td class="pr-6 text-right">
                                        <div class="flex justify-end items-center gap-3">
                                            ...
                                            <form method="POST" onsubmit="return confirmAction(event, 'Silence Voice?', 'This testimony will be removed from the registry.');" class="inline">
